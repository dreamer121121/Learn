...
def insert(db, cursor, ip, port, banner) :
    ...
    if count != 0: #old
        addTo_instance = "update knowledgeBase_instance set ip = '%s', update_time = '%s', from_spider = %d where ip = '%s'" % (ip, timestamp, from_spider, ip)
        ...
        addTo_instance = "INSERT INTO knowledgeBase_instance(name, ip, timestamp, from_spider) VALUES ('%s', '%s', '%s', %d)" % (name, ip, timestamp, from_spider)
        addTo_instanceport = "INSERT INTO knowledgeBase_instanceport(id, ip, port, banner, add_time, update_time, instance_id, protocol) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (id, ip, port, banner, timestamp, timestamp, instance_id, protocol)
    try:
...
if __name__ == "__main__" :
    opts, args = getopt.getopt(sys.argv[1: ], "i:u:")
