...
int http_tcpclient_connect(int socket_fd, char* host, int port, struct hostent *he, struct sockaddr_in *server_addr) {
    ...
    // }
    // if (he->h_addr == NULL) {
    //     cout << "something strange, the struct hostent generated by function gethostname is not null, but its member points is all null." << endl;
    //     return -1;
    // }

    // he->h_addr_list = (char**)malloc(sizeof(100));
    // *(he->h_addr_list) = (char*)malloc(sizeof(100));
    // memset(he->h_addr_list[0], '\0', 100);
    // strcpy(he->h_addr_list[0], host);
    // cout << he->h_addr_list[0] << endl;
    server_addr->sin_family = AF_INET;
    server_addr->sin_port = htons(port);
    if (inet_pton(AF_INET, host, &server_addr->sin_addr) < 0 ) {
        cout << "the host is wrong. " << host << endl;
        return -1;
    }
    ...
    if(connect(socket_fd, (struct sockaddr *)server_addr,sizeof(struct sockaddr)) == -1) {
        // cout << "http_tcpclient_connect failed" << endl;
        return -1;
    }
